...
{
    return page2ppn(page) << PGSHIFT; // 先找到页帧号，一个页帧4KB，然后*4096就是物理地址
}
...
static inline void *page2kva(struct Page *page) { return KADDR(page2pa(page)); } // 将物理地址转换为虚拟地址
...
{
    return (ppn << PTE_PPN_SHIFT) | PTE_V | type; // 左移10位，然后加上PTE_V和type
}
