...
// 从一个页表项中获取对应的物理地址，与～0x3FF按位与是为了清除页表项的标志位，然后左移动12-10位，即可得到物理地址
#define PTE_ADDR(pte) (((uintptr_t)(pte) & ~0x3FF) << (PTXSHIFT - PTE_PPN_SHIFT))
#define PDE_ADDR(pde) PTE_ADDR(pde)

/* page directory and page table constants */
#define NPDEENTRY 512 // page directory entries per page directory 每个页目录表中的页目录项数
#define NPTEENTRY 512 // page table entries per page table 每个页表中的页表项数
